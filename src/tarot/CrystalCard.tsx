...
const palettes: Record<Suit | 'major', { a: string; b: string; c: string }> = {
  major: { a: 'var(--cg-lavender)', b: 'var(--cg-cyan)', c: 'var(--cg-gold-bright)' },
  wands: { a: 'var(--cg-amber)', b: 'var(--cg-flame)', c: 'var(--cg-gold-bright)' },
  cups: { a: 'var(--cg-cyan)', b: 'var(--cg-blue)', c: 'var(--cg-lavender)' },
  swords: { a: 'var(--cg-silver)', b: 'var(--cg-ice)', c: 'var(--cg-lavender)' },
  pentacles: { a: 'var(--cg-emerald)', b: 'var(--cg-teal)', c: 'var(--cg-gold)' },
}
...
function Star({ x, y, r = 8, points = 4, color = '#ecd49b', opacity = .9 }: { x: number; y: number; r?: number; points?: number; color?: string; opacity?: number }) {
  const inner = r * .28
...
function SuitSigil({ suit, x, y, size = 1, colors }: { suit: Suit; x: number; y: number; size?: number; colors: typeof palettes.major }) {
  return <g transform={`translate(${x} ${y}) scale(${size})`} className={`sigil sigil-${suit}`} stroke="var(--gold-url)" strokeWidth="1.1" fill="none" strokeLinejoin="round">
...
function Halo({ cx = 120, cy = 201, r = 68, colors, orbit = false }: { cx?: number; cy?: number; r?: number; colors: typeof palettes.major; orbit?: boolean }) {
  return <g className={orbit ? 'orbiting-halo' : undefined}>
    <circle cx={cx} cy={cy} r={r} fill="none" stroke="var(--gold-url)" strokeWidth=".9" opacity=".73"/>
    ...
    {Array.from({ length: 12 }, (_, i) => { const a = i * Math.PI / 6; return <path key={i} d={`M${cx + Math.cos(a) * (r - 4)} ${cy + Math.sin(a) * (r - 4)}L${cx + Math.cos(a) * (r + 3)} ${cy + Math.sin(a) * (r + 3)}`} stroke="#e5cb91" strokeWidth=".8" opacity=".8"/> })}
  </g>
}

function CrystalOrb({ x, y, r, colors }: { x: number; y: number; r: number; colors: typeof palettes.major }) {
  return <g>
    <circle cx={x} cy={y} r={r + 4} fill="none" stroke="var(--gold-url)" strokeWidth=".9"/>
...
function MajorScene({ card, colors }: { card: DeckCard; colors: typeof palettes.major }) {
  const id = card.id
  const triangle = <path d="M120 105 184 232 56 232Z" fill="none" stroke="var(--gold-url)" strokeWidth="1" opacity=".68"/>
  ...
  if (id === 'the-hermit') return <>
    ...
    <Halo cy={175} r={45} colors={colors}/><path d="M118 96V154M118 112 151 130" stroke="var(--gold-url)" strokeWidth="2"/>
    <path d="M149 122 159 130 150 139 141 130Z" fill="#ffe6a8" className="lantern-glow"/><circle cx="150" cy="130" r="20" fill={colors.c} opacity=".12" className="lantern-glow"/>
    <Crystal x={120} y={197} size={27} colors={colors}/><path d="M92 282H148M100 289H140" stroke="var(--gold-url)"/>
  </>
  if (id === 'the-tower') return <>
    <path d="M85 263V155L94 155 94 134 108 134 108 113 132 113 132 134 146 134 146 155 155 155V263Z" fill="#171b3f" stroke="var(--gold-url)" strokeWidth="1.4"/>
  ...
  if (id === 'the-star') return <>
    ...
    <Star x={120} y={180} r={34} points={8} color="#e1d2ff"/><Star x={120} y={180} r={19} points={8} color={colors.c}/><circle cx="120" cy="180" r="5" fill="#fff4d1" className="star-core"/>
    {Array.from({length:7},(_,i)=>{const a=i*Math.PI*2/7-Math.PI/2;return <g key={i}><path d={`M120 180L${120+Math.cos(a)*56} ${183+Math.sin(a)*61}`} stroke={colors.a} strokeOpacity=".22"/><Star x={120+Math.cos(a)*59} y={183+Math.sin(a)*64} r={i%2?6:8} color={i%2?colors.c:'#ecd49b'} opacity={.9}/></g>})}
  ...
  if (id === 'the-emperor') return <><path d="M68 250 68 157 120 127 172 157 172 250Z" fill="#182044" stroke="var(--gold-url)" strokeWidth="1.4"/><path d="M82 239V168L120 147 158 168V239ZM95 222V182L120 168 145 182V222Z" fill="none" stroke={colors.b}/><path d="M120 105 134 132 120 145 106 132Z" fill="var(--glass-url)" stroke={colors.c}/><path d="M75 261H165M84 270H156" stroke="var(--gold-url)"/><Halo cy={190} r={72} colors={colors}/></>
  if (id === 'the-hierophant') return <><path d="M64 266V180Q64 120 120 112Q176 120 176 180V266M83 266V185Q83 139 120 133Q157 139 157 185V266" fill="none" stroke="var(--gold-url)" strokeWidth="1.3"/><path d="M89 190 120 159 151 190 120 221Z" fill="var(--glass-url)" stroke={colors.b}/><path d="M120 221V260M107 243H133" stroke={colors.c}/><CircleGeometry colors={colors}/></>
  if (id === 'the-lovers') return <><Halo cy={188} r={61} colors={colors}/><Crystal x={82} y={193} size={37} rotation={-18} colors={colors}/><Crystal x={158} y={193} size={37} rotation={18} colors={colors}/><Star x={120} y={177} r={18} points={8} color={colors.c}/><path d="M82 228Q120 254 158 228M90 240Q120 260 150 240" fill="none" stroke={colors.a}/><path d="M82 145 120 127 158 145" fill="none" stroke="var(--gold-url)"/></>
  if (id === 'the-chariot') return <><path d="M71 220 91 164H149L169 220 153 244H87Z" fill="#182143" stroke="var(--gold-url)"/><path d="M80 224H160M94 164V145H146V164" fill="none" stroke={colors.c}/><Crystal x={120} y={197} size={34} colors={colors} glow/><path d="M54 242 89 228M186 242 151 228" stroke={colors.a} strokeWidth="2"/><path d="M61 231 53 242 67 239M179 231 187 242 173 239" fill={colors.a}/><circle cx="78" cy="257" r="11" fill="none" stroke="var(--gold-url)"/><circle cx="162" cy="257" r="11" fill="none" stroke="var(--gold-url)"/></>
  if (id === 'strength') return <><path d="M78 162C75 131 99 125 120 147 141 125 165 131 162 162 158 195 82 195 78 162Z" fill="var(--glass-url)" stroke={colors.c}/><path d="M81 165Q120 149 159 165M120 151V184" fill="none" stroke="var(--gold-url)"/><path d="M79 205C54 177 58 147 120 147 182 147 186 177 161 205 186 231 171 258 120 225 69 258 54 231 79 205Z" fill="none" stroke={colors.a} className="orbiting-halo"/><Star x={120} y={184} r={13} color={colors.c}/></>
  if (id === 'wheel-of-fortune') return <><Halo cy={192} r={84} colors={colors} orbit/><g className="wheel-inner"><circle cx="120" cy="192" r="57" fill="#111a38" stroke="var(--gold-url)" strokeWidth="1.4"/><circle cx="120" cy="192" r="43" fill="none" stroke={colors.b}/>{Array.from({length:8},(_,i)=><g key={i} transform={`rotate(${i*45} 120 192)`}><path d="M120 149 128 185 120 192 112 185Z" fill="var(--glass-url)" stroke={colors.c}/></g>)}<Crystal x={120} y={192} size={23} colors={colors}/></g>{[0,1,2,3].map(i=><Star key={i} x={[120,202,120,38][i]} y={[99,192,285,192][i]} r={7} color={colors.c}/>)}</>
  if (id === 'justice') return <><path d="M120 118V260M86 143H154M94 143 74 185H114ZM146 143 126 185H166Z" fill="none" stroke="var(--gold-url)" strokeWidth="1.3"/><path d="M74 185Q94 207 114 185M126 185Q146 207 166 185M104 260H136M93 270H147" fill="none" stroke={colors.b}/><Crystal x={120} y={161} size={21} colors={colors}/><path d="M120 104 129 119 120 133 111 119Z" fill={colors.c}/><Halo cy={186} r={69} colors={colors}/></>
  if (id === 'the-hanged-man') return <><path d="M73 127H167M83 127V259M157 127V259M83 146H157" fill="none" stroke="var(--gold-url)"/><g transform="rotate(180 120 193)"><Crystal x={120} y={190} size={47} colors={colors} glow/><path d="M120 150V228M98 167 142 213M142 167 98 213" fill="none" stroke={colors.b}/></g><Halo cy={191} r={64} colors={colors} orbit/><path d="M96 259H144" stroke={colors.c}/></>
  ...
  if (id === 'temperance') return <><path d="M85 156H111L108 185Q98 194 88 185ZM129 208H155L152 237Q142 246 132 237Z" fill="var(--glass-url)" stroke="var(--gold-url)"/><path d="M102 186C135 189 105 205 139 209" fill="none" stroke={colors.c} strokeWidth="2" strokeDasharray="3 3" className="energy-flow"/><Halo cy={190} r={63} colors={colors}/><Crystal x={120} y={195} size={27} colors={colors}/><path d="M89 151 107 151M133 203 151 203" stroke={colors.c}/></>
  ...
  if (id === 'the-moon') return <><circle cx="120" cy="152" r="35" fill={colors.c} opacity=".15"/><path d="M126 119A35 35 0 1 0 126 185A25 25 0 1 1 126 119Z" fill="#e8d5a8"/><path d="M57 259V191Q57 164 80 164T103 191V259ZM137 259V191Q137 164 160 164T183 191V259Z" fill="#121932" stroke="var(--gold-url)"/><path d="M47 259H113M127 259H193M120 203Q85 220 120 249Q155 220 120 203Z" fill="none" stroke={colors.b}/><path d="M44 217Q82 207 109 218M131 219Q158 207 196 216" fill="none" stroke={colors.c} opacity=".5" className="lunar-mist"/></>
  if (id === 'the-sun') return <><g className="sun-rays">{Array.from({length:16},(_,i)=><path key={i} d="M120 176V112" transform={`rotate(${i*22.5} 120 176)`} stroke={i%2?colors.a:colors.c} strokeWidth={i%2?1:2} opacity=".76"/>)}</g><circle cx="120" cy="177" r="42" fill="var(--orb-url)" stroke="var(--gold-url)"/><Star x={120} y={177} r={25} points={8} color="#ffe7a2"/><circle cx="120" cy="177" r="7" fill="#fff4cd"/><path d="M55 255Q120 229 185 255M65 265H175" fill="none" stroke={colors.b}/></>
  if (id === 'judgement') return <><path d="M120 116V167M102 132H138M108 153H132" fill="none" stroke="var(--gold-url)" strokeWidth="2"/><Star x={120} y={115} r={15} points={8} color={colors.c}/>{[-1,0,1].map((v)=><g key={v} transform={`translate(${v*39} 0)`}><path d={`M${120+v*0} 254 111 218 116 187 120 174 124 187 129 218Z`} fill="var(--glass-url)" stroke={colors.b}/><path d={`M${120} 231V189`} stroke={colors.c}/></g>)}<path d="M78 264Q120 238 162 264" fill="none" stroke={colors.a}/></>
  return <><Halo cy={191} r={77} colors={colors} orbit/><circle cx="120" cy="191" r="59" fill="none" stroke={colors.b}/><circle cx="120" cy="191" r="43" fill="none" stroke={colors.c} strokeDasharray="2 4"/><Crystal x={120} y={191} size={35} colors={colors} glow/><path d="M120 105V125M120 257V277M34 191H54M186 191H206M59 130 73 144M167 238 181 252M181 130 167 144M73 238 59 252" stroke="var(--gold-url)" strokeWidth="1.4"/>{[0,1,2,3].map(i=><SuitSigil key={i} suit={(['wands','cups','swords','pentacles'] as Suit[])[i]} x={[120,194,120,46][i]} y={[116,191,266,191][i]} size={.26} colors={palettes[(['wands','cups','swords','pentacles'] as Suit[])[i]]}/>)}</>
}

function CircleGeometry({ colors }: { colors: typeof palettes.major }) { return <><circle cx="120" cy="192" r="51" fill="none" stroke={colors.b}/><path d="M120 141 164 217H76Z" fill="none" stroke="var(--gold-url)"/><circle cx="120" cy="192" r="10" fill={colors.c} fillOpacity=".25" stroke={colors.c}/></> }
...
function PipScene({ card, colors }: { card: DeckCard; colors: typeof palettes.major }) {
  ...
  return <>
    ...
    {layout === 'gateway' && <><path d="M65 270V188Q65 128 120 128T175 188V270M81 270V190Q81 145 120 145T159 190V270" fill="none" stroke="var(--gold-url)"/></>}
...
function CourtScene({ card, colors }: { card: DeckCard; colors: typeof palettes.major }) {
  ...
  return <>
    <path d={rank === 'knight' ? 'M57 257 172 133M75 270 190 146' : 'M62 262V178Q62 132 120 118Q178 132 178 178V262'} fill="none" stroke="var(--gold-url)" strokeWidth={rank === 'king' ? 1.5 : 1}/>
    ...
    {rank === 'king' && <><path d="M84 153 98 126 120 145 142 126 156 153V172H84Z" fill="#171a38" stroke="var(--gold-url)"/><circle cx="98" cy="128" r="3" fill={colors.c}/><circle cx="120" cy="145" r="3" fill={colors.c}/><circle cx="142" cy="128" r="3" fill={colors.c}/><path d="M85 251H155M93 260H147" stroke={colors.a}/></>}
...
export function CrystalCard({ card, reversed = false, animations = true, className = '', reveal = false }: { card: DeckCard; reversed?: boolean; animations?: boolean; className?: string; reveal?: boolean }) {
  ...
  return <svg ref={cardRef} className={`crystal-card ${motionActive ? 'motion-on' : 'motion-off'} ${reveal ? 'card-reveal' : ''} ${className}`} data-animation={animationName} style={{ '--card-phase': phase } as CSSProperties} viewBox="0 0 240 384" role="img" aria-label={`${card.name}${reversed ? ', reversed' : ''}`} preserveAspectRatio="xMidYMid meet">
    <defs>
      ...
      <linearGradient id={`gold-${uid}`}><stop stopColor="#9b7744"/><stop offset=".5" stopColor="#f0d89f"/><stop offset="1" stopColor="#9b7744"/></linearGradient>
    ...
    <g className={reversed ? 'whole-card reversed-art' : 'whole-card'}>
      ...
      <g className="ambient-stars" opacity=".8">{Array.from({length:20},(_,i)=>{const x=32+((i*53+17)%176), y=51+((i*79+23)%278);return <circle key={i} cx={x} cy={y} r={i%5===0?1.1:.55} fill={i%3===0?colors.c:'#e9d9b2'} style={{'--twinkle-delay':`${((i*1.73)%8).toFixed(2)}s`} as CSSProperties}/>})}</g>
      <path d="M29 58 39 48 49 58 39 68Z M191 58 201 48 211 58 201 68Z M29 326 39 316 49 326 39 336Z M191 326 201 316 211 326 201 336Z" fill="none" stroke={`url(#gold-${uid})`} strokeWidth=".9"/>
      <path d="M39 90V71M201 90V71M39 294V313M201 294V313" stroke={`url(#gold-${uid})`} strokeWidth=".7" opacity=".7"/>
      <path d="M48 49Q120 35 192 49M48 335Q120 349 192 335" fill="none" stroke={`url(#gold-${uid})`} strokeWidth=".55" opacity=".65"/>
      <g fill="none" stroke={`url(#gold-${uid})`} strokeWidth=".65" opacity=".35">
      ...
      <g className={`card-scene card-${uid} suit-${card.suit ?? 'major'}`} style={{'--accent':colors.a} as CSSProperties}>
        ...
        <g className="scene-engraving" fill="none" stroke={`url(#gold-${uid})`} strokeWidth=".65" opacity=".29">
        ...
        <g style={{'--glass-url':`url(#glass-${uid})`,'--gold-url':`url(#gold-${uid})`,'--orb-url':`url(#orb-${uid})`} as CSSProperties}>{scene}</g>
      </g>
      <path d="M47 310H193" stroke={`url(#gold-${uid})`} strokeOpacity=".6" strokeWidth=".7"/>
      <path d="M55 316 61 322 67 316M173 316 179 322 185 316" fill="none" stroke={colors.c} strokeWidth=".8"/>
      <text x="120" y="337" textAnchor="middle" className="card-title" style={{ fontSize: card.name.length > 15 ? 6.4 : 8.1 }}>{card.name.toUpperCase()}</text>
      {card.suit && <text x="120" y="352" textAnchor="middle" className="card-suit-label">{card.suit.toUpperCase()}</text>}
      <path d="M97 365H143" stroke={`url(#gold-${uid})`} strokeWidth=".7"/><circle cx="120" cy="365" r="2" fill={colors.c}/>
    </g>
  </svg>
}
